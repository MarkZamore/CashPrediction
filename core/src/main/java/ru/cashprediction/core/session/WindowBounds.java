 ...
 *
 * <p>Сообщения проверок берутся из каталога текстов: при чтении повреждённого снимка они попадают в отчёт
 * восстановления.</p>
 *
...
public record WindowBounds(double x, double y, double width, double height) {
    ...
    public WindowBounds {
        if (!Double.isFinite(x) || !Double.isFinite(y) || !Double.isFinite(width) || !Double.isFinite(height)) {
            throw new IllegalArgumentException(Texts.get("session.bounds.error.notFinite"));
        }
        if (width < 0 || height < 0) {
            throw new IllegalArgumentException(Texts.get("session.bounds.error.negativeSize", width, height));
        }
    }
}
