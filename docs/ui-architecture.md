...
- **L13. Все тексты — в одном каталоге.** `core/src/main/resources/ru/cashprediction/core/ui/text/<область>_ru.properties` (UTF-8), поиск `<область>_<язык>` с запасным `<область>`, язык зафиксирован `ru`, английских файлов и переключения языка нет. Поиск текста — `core.text.Texts` (без зависимостей, доступен любому слою ядра) и `core.ui.text.UiText` (обёртка для моделей интерфейса). Новый код не содержит кириллических литералов (`NoCyrillicLiteralsInNewCodeTest`); прежние ~695 литералов ядра переносит этап S0.5. Ключевые слова формата файлов плана и web-сеанса — грамматика данных: S0.5 переносит их в нелокализуемый ресурс без суффикса языка. Строгий режим `-Dcashprediction.ui.strictText=true` (включён в тестах ядра): отсутствующий ключ — исключение, у пользователя — `!ключ!`.

### Список переноса S0.5, пополненный на этапе S0

Кириллические литералы, добавленные или изменённые в S0 вне проверяемых `NoCyrillicLiteralsInNewCodeTest` пакетов:
...
- `core/src/test/java/ru/cashprediction/core/text/JavaSourceScanner` — литералы исходников без комментариев: проверка кириллицы в новом коде и ссылок на ключи каталога (`UiTextCatalogTest`; проверка неиспользуемых ключей включается свойством `cashprediction.catalog.requireAllUsed=true` с этапа S2).
